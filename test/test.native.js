...
var dvariancech = tryRequire( resolve( __dirname, './../lib/native.js' ) );
var opts = {
...
tape( 'main export is a function', opts, function test( t ) {
	...
	t.strictEqual( typeof dvariancech, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function has an arity of 1', opts, function test( t ) {
	t.strictEqual( dvariancech.length, 1, 'has expected arity' );
	t.end();
});

tape( 'the function calculates the variance of a one-dimensional ndarray', opts, function test( t ) {
	var correction;
	var expected;
	var popts;
	var x;
	...
	correction = scalar2ndarray( 1.0, popts );

	v = dvariancech( [ vector( x, x.length, 1, 0 ), correction ] );
...
tape( 'if provided an empty ndarray, the function returns `NaN`', opts, function test( t ) {
	...
	v = dvariancech( [ vector( x, 0, 1, 0 ), correction ] );
	t.strictEqual( isnan( v ), true, 'returns expected value' );
...
tape( 'if provided a correction argument yielding `N-correction` less than or equal to `0`, the function returns `NaN`', opts, function test( t ) {
	...
	v = dvariancech( [ vector( x, 1, 1, 0 ), correction ] );
	t.strictEqual( isnan( v ), true, 'returns expected value' );
...
tape( 'the function supports one-dimensional ndarrays having non-unit strides', opts, function test( t ) {
	...
	v = dvariancech( [ vector( x, 4, 2, 0 ), correction ] );
	expected = 6.25;
	t.strictEqual( v, expected, 'returns expected value' );
...
tape( 'the function supports one-dimensional ndarrays having negative strides', opts, function test( t ) {
	...
	v = dvariancech( [ vector( x, 4, -2, 6 ), correction ] );
	expected = 6.25;
	t.strictEqual( v, expected, 'returns expected value' );
...
tape( 'the function supports one-dimensional ndarrays having non-zero offsets', opts, function test( t ) {
	...
	v = dvariancech( [ vector( x, 4, 2, 1 ), correction ] );
	expected = 6.25;
	t.strictEqual( v, expected, 'returns expected value' );
